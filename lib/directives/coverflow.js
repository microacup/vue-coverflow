...
export default {
    bind: function (el, binding, vnode) {
        ...
        if (shadow) {
            ...
            el.style.height = (imgHeight + 120) + 'px'
        }
        ...
        function handleClick(event) {
            ...
            while (target.className !== 'cover-card' && !target.classList.contains('cover-card')) {
                target = target.parentNode;
            }
            if (target) {
                var index = imgs.indexOf(target)
                displayIndex(imgSize, spacing, el.scrollLeft, imgs, index, flat, parseInt(el.style.width), titleBox, vnode);
            }
        }
